...
class search_it
{
    ...
    private static function tablePrefix() {
    ...
    private static function tempTablePrefix() {
    ...
    public function generateIndex()
    {
        ...
        $art_sql->setTable(self::tablePrefix() . 'article');
        ...
        if ($this->indexMediapool) {
            ...
            $mediaSQL->setTable(self::tablePrefix() . 'media');
    ...
    public function indexArticle($_id, $_clang = false)
    {
        ...
        foreach ($langs as $lang) {
            ...
            $where = sprintf("ftable = '%s' AND fid = %d AND clang = %d", self::tablePrefix() . 'article', $_id, $langID);

            // delete from cache
            $select = rex_sql::factory();
            $select->setTable(self::tempTablePrefix() . 'search_it_index');
            ...
            // delete old
            $delete->setTable(self::tempTablePrefix() . 'search_it_index');
            ...
            if (is_object($article) AND ($article->isOnline() OR rex_addon::get('search_it')->getConfig('indexoffline')) AND $_id != 0
                ...
                $articleData['ftable'] = self::tablePrefix() . 'article';
                ...
                if (array_key_exists(self::tablePrefix() . 'article', $this->includeColumns)) {
                    $additionalValues = [];
                    $select->flushValues();
                    $select->setTable(self::tablePrefix() . 'article');
                    $select->setWhere('id = ' . $_id . ' AND clang_id = ' . $langID);
                    $select->select('`' . implode('`,`', $this->includeColumns[self::tablePrefix() . 'article']) . '`');
                    foreach ($this->includeColumns[self::tablePrefix() . 'article'] as $col) {
                ...
                $insert->setTable(self::tempTablePrefix() . 'search_it_index');
    ...
    public function unindexArticle($_id, $_clang = false)
    {
        // exclude article
        $art_sql = rex_sql::factory();
        $art_sql->setTable(self::tempTablePrefix() . 'search_it_index');
    ...
    public function indexColumn($_table, $_column, $_idcol = false, $_id = false, $_start = false, $_count = false, $_wherecondition = false)
    {
        ...
        if ($sql->select('*')) {
            ...
            foreach ($sql->getArray() as $row) {
                if (!empty($row[$_column]) AND ( rex_addon::get('search_it')->getConfig('indexoffline') OR self::tablePrefix() . 'article' != $_table OR $row['status'] == '1')
                    AND (self::tablePrefix() . 'article' != $_table OR !in_array($row['id'], $this->excludeIDs))
                ) {
                    $insert = rex_sql::factory();
                    $indexData = [];
                    ...
                    } elseif ($_table == self::tablePrefix() . 'article') {
                    ...
                    if (array_key_exists('parent_id', $row)) {
                        ...
                        if ($_table == self::tablePrefix() . 'article') {
                    ...
                    $indexData['teaser'] = '';
                    if (self::tablePrefix() . 'article' == $_table) {
    ...
    public function indexFile($_filename, $_doPlaintext = false, $_clang = false, $_fid = false, $_catid = false)
    {
        ...
        if ($_fid !== false) {
            $fileData['ftable'] = self::tablePrefix() . 'media';
        }
    ...
    {
        $delete = rex_sql::factory();
		$delete->setQuery('TRUNCATE '. self::tempTablePrefix() .'search_it_index');
    ...
    {
        $sql = rex_sql::factory();
		$query = 'SELECT id FROM '. self::tempTablePrefix() .'search_it_index WHERE texttype = "'. $texttype .'";';
		$deleteIds = [];
        foreach ($sql->getArray($query) as $cacheId) {
			$deleteIds[] = $cacheId['id'];
		}
		// Delete index
		$sql->setQuery('DELETE FROM '. self::tempTablePrefix() .'search_it_index WHERE texttype = "'. $texttype .'"');
		// Delete cache
    ...
    {
        $sql = rex_sql::factory();
        $sql->setTable(self::tempTablePrefix() . 'search_it_cache');
    ...
    private function cacheSearch($_result, $_indexIds)
    {
        ...
        if (isset($Ainsert) && implode(',', $Ainsert) != '') {
            ...
            %s;',
                        self::tempTablePrefix() . 'search_it_cacheindex_ids',
                        implode(',', $Ainsert)
                    )
                );
                $info = 'Success';
    ...
    public function deleteCache($_indexIds = false)
    {
        if ($_indexIds === false) {
            ...
            $delete = rex_sql::factory();
            $delete->setQuery('TRUNCATE '. self::tempTablePrefix() . 'search_it_cacheindex_ids');
            $delete->setQuery('TRUNCATE '. self::tempTablePrefix() . 'search_it_cache');
            ...
            $delete = rex_sql::factory();
            $delete->setTable(self::tempTablePrefix() . 'search_it_cache');
            $delete->setWhere('id IN (' . implode(',', $deleteIds) . ')');
            $delete->delete();

            // delete the cache-ID and index-ID
            $delete2 = rex_sql::factory();
            $delete2->setTable(self::tempTablePrefix() . 'search_it_cacheindex_ids');
            ...
            $delete3->setTable(self::tempTablePrefix() . 'search_it_cache');
            $delete3->setWhere(sprintf('id NOT IN (SELECT cache_id FROM `%s`)', self::tempTablePrefix() . 'search_it_cacheindex_ids'));
            $delete3->delete();
        }
    }
        ...
        if (!empty($simWords)) {
            $simWordsSQL->setQuery(
              ...
              ON DUPLICATE KEY UPDATE count = count + %d",
                    self::tempTablePrefix() . 'search_it_keywords',
                    implode(',', $simWords),
                    $_doCount ? 1 : 0
                )
            );
    ...
    public function deleteKeywords()
    {
        ...
        return $kw_sql->setQuery(sprintf('TRUNCATE TABLE `%s`', self::tempTablePrefix() . 'search_it_keywords'));
    }
    ...
    function search($_search)
    {
        ...
        if (array_key_exists('categories', $this->searchInIDs) AND count($this->searchInIDs['categories'])) {
            $AwhereToSearch[] = "(catid IN (" . implode(',', $this->searchInIDs['categories']) . ") AND ftable = '" . self::tablePrefix() . "article')";
        }

        if (array_key_exists('filecategories', $this->searchInIDs) AND count($this->searchInIDs['filecategories'])) {
            $AwhereToSearch[] = "(catid IN (" . implode(',', $this->searchInIDs['filecategories']) . ") AND ftable = '" . self::tablePrefix() . "media')";
        }
        ...
        $selectFields = [];
        if ($this->groupBy) {
            $selectFields[] = sprintf('(SELECT SUM%s FROM `%s` summe WHERE summe.fid = r1.fid AND summe.ftable = r1.ftable) AS RELEVANCE_SEARCH_IT', $match, self::tempTablePrefix() . 'search_it_index');
            $selectFields[] = sprintf('(SELECT COUNT(*) FROM `%s` summe WHERE summe.fid = r1.fid AND (summe.ftable IS NULL OR summe.ftable = r1.ftable) AND (summe.fcolumn IS NULL OR summe.fcolumn = r1.fcolumn) AND summe.texttype = r1.texttype) AS COUNT_SEARCH_IT', self::tempTablePrefix() . 'search_it_index');
        } else {
        ...
        if ($this->groupBy) {
            ...
            LIMIT %d,%d',
                implode(",\n", $selectFields),
                self::tempTablePrefix() . 'search_it_index',
                $where,
                $match,
                $match,
                self::tempTablePrefix() . 'search_it_index',
