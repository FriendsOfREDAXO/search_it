        ...
        switch(rex_get('type')){
            ...
            case 'col':
                if(false !== ($count = $search_it->indexColumn(rex_get('t'), rex_get('c'), false, false, rex_get('s'), rex_get('w')))) {
                    echo '<p class="text-info"><em>`' . rex_get('t') . '`.`' . rex_get('c') . '` (' . $this->i18n('search_it_generate_col_datasets').' '.rex_get('s') . ' - ' . (rex_get('s') + rex_get('w')) . ')</em> '. $this->i18n('search_it_generate_col_done',$count) . '</p>';
                } else {
