...
$path = rex_path::plugin('search_it','documentation','docs/'.$lang.'/');
...
if ( rex_request("search_it_docs_image","string") != "" && isset($files[rex_request("search_it_docs_image","string")]) ) {
    ob_end_clean();
    $content = rex_file::get($path.basename(rex_request("search_it_docs_image","string")));
    echo $content;
    exit;
...
$file = rex_request('search_it_docs_file','string','search_it_intro.md');
if (!in_array($file, $files)) {
    $file = 'search_it_intro.md';
}
$content = rex_file::get($path.basename($file));
if ($content == "") {
    $content = '<p class="alert alert-warning">'.rex_i18n::rawMsg('search_it_docs_filenotfound').'</p>';
}
