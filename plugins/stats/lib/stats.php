...
class search_it_stats{
    ...
    function flushSQL(){
        ...
        $this->sql->setTable(rex::getTablePrefix().'searchit_stats_searchterms');
    }
    ...
    function truncate(){
        $this->sql->setQuery('TRUNCATE ' . rex::getTablePrefix().'searchit_stats_searchterms');
    }
    ...
    function getTopSearchterms($_count, $_getonly = 0){
        ...
        if (empty($_getonly)){
            $query = 'SELECT term, COUNT(*) as count, 1 as success FROM `' . rex::getTablePrefix().'searchit_stats_searchterms' . '` WHERE resultcount > 0 GROUP BY term
      UNION
      SELECT term, COUNT(*) as count, 0 as success FROM `' . rex::getTablePrefix().'searchit_stats_searchterms' . '` WHERE resultcount <= 0 GROUP BY term';
        } else {
            $query = 'SELECT term, COUNT(*) as count, ' . ($_getonly == 1 ? 1 : 0) . ' as success FROM `' . rex::getTablePrefix().'searchit_stats_searchterms' . '` WHERE resultcount ' . ($_getonly == 1 ? '>' : '<=') . ' 0 GROUP BY term';
